...
namespace collision{
    ...
    public:
              ...
              number_of_vertices_(tunnel_parameters.num_of_points)
        {
            ...
            Eigen::RowVectorXf elbow_x = -tunnel_parameters.lu * (t2.array().cos() * t1.array().sin());
            Eigen::RowVectorXf elbow_y = tunnel_parameters.lu * t2.array().sin();
            Eigen::RowVectorXf elbow_z = -tunnel_parameters.lu * (t1.array().cos() * t2.array().cos());

            Eigen::Matrix<float, 3, Eigen::Dynamic> elbow(3, tunnel_parameters.num_of_points);
            elbow.row(0) = elbow_x;
            elbow.row(1) = elbow_y;
            elbow.row(2) = elbow_z;
