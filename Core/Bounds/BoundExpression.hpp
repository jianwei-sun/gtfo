...
class BoundExpression : public BooleanExpression<std::shared_ptr<BoundBase<Dimensions, Scalar>>>{
    ...
    using BooleanExpression = BooleanExpression<BoundPtr>;
...
public:
    ...
    BoundExpression(const Relation& relation, const Types&... types) 
        : BooleanExpression(relation, types...) {}
    ...
    operator&(const LeftType& lhs, const RightType& rhs){
        ...
        if constexpr(std::is_same_v<BoundExpression, LeftType> && std::is_same_v<BoundExpression, RightType>){
        ...
        // Expression & Bound
        if constexpr(std::is_same_v<BoundExpression, LeftType> && std::is_base_of_v<BoundBase, RightType>){
            return BoundExpression(Relation::Intersection, 
                lhs, 
        ...
        // Bound & Expression
        if constexpr(std::is_base_of_v<BoundBase, LeftType> && std::is_same_v<BoundExpression, RightType>){
            return BoundExpression(Relation::Intersection, 
    ...
    [[nodiscard]] virtual bool Contains(const VectorN& point) const {
        const std::function<bool(const BoundPtr&)> map = [&](const BoundPtr& ptr)->bool{
