...
namespace gtfo
{
    ...
    class OrientationBound : public BoundBase<Dimensions, Scalar>
    {
    public:
        ...
        // roll - pitch - yaw
        OrientationBound(const VectorN &lower_limits, const VectorN &upper_limits, const VectorN &center, const Scalar& tol = GTFO_EQUALITY_COMPARISON_TOLERANCE)
        ...
        // Case where we want same limit distance in upper and lower bounds
        OrientationBound(const VectorN &bilateral_limits, const VectorN& center = VectorN::Zero(), const Scalar& tol = GTFO_EQUALITY_COMPARISON_TOLERANCE)
            : BoundBase<Dimensions, Scalar>(tol),
        ...
        [[nodiscard]] BoundPtr DeepCopy(void) const override {
            return std::make_shared<OrientationBound>(*this);
        }
